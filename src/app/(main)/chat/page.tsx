...
export default function ChatPage() {
    ...
    const sendMessage = () => {
        ...
        setTimeout(() => {
             ...
             typingElement.innerHTML = `
                <div class="p-4 rounded-2xl text-sm bg-black/30 backdrop-blur-sm text-slate-400 border border-white/10 rounded-tl-none flex gap-1 items-center">
             ...
             setTimeout(() => {
                ...
                responseElement.innerHTML = `
                    <div class="max-w-[85%] md:max-w-[70%] p-4 rounded-2xl text-sm bg-black/30 backdrop-blur-sm text-slate-200 border border-white/10 rounded-tl-none">
  ...
  return (
    <div className="flex h-[calc(100vh-12rem)] gap-6">
        <div className="flex-1 flex flex-col overflow-hidden bg-black/20 border border-white/10 rounded-2xl relative shadow-2xl backdrop-blur-lg">
            <div className="p-4 border-b border-white/10 bg-black/30 backdrop-blur-lg flex items-center gap-3">
                ...
                <div>
                    <p className="text-sm font-bold text-white">QORE Chat</p>
            ...
            <div className="flex-1 overflow-y-auto p-8 pt-6 space-y-6 custom-scrollbar" id="chat-messages">
                <div className="flex justify-start">
                    <div className="max-w-[85%] md:max-w-[70%] p-4 rounded-2xl text-sm bg-black/30 backdrop-blur-sm text-slate-200 border border-white/10 rounded-tl-none">Willkommen bei der QORE Kommunikation. Wie kann ich heute helfen?</div>
                </div>
            </div>
            <div className="p-4 bg-black/20 border-t border-white/10">
                <div className="max-w-4xl mx-auto flex gap-3">
                    <input type="text" id="chat-input" placeholder="Frag deine QORE..." className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 transition-colors" onKeyDown={(e) => e.key === 'Enter' && sendMessage()} />
