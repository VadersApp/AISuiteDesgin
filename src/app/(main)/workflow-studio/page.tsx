        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-300">
                {filteredWorkflows.map(wf => {
                    ...
                    return (
                        <Card key={wf.id} className="p-6 flex flex-col justify-between hover:border-primary/50 transition-all group">
                           <div>
                             <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-foreground leading-tight line-clamp-2 group-hover:text-primary transition-colors">{wf.name}</h3>
                                    </div>
                                </div>
                            ...
                            <p className="text-xs text-muted-foreground mb-6 line-clamp-3 leading-relaxed">{wf.description}</p>
                           </div>

                            <div className="mt-auto pt-4 border-t border-border space-y-3 text-xs text-muted-foreground">
                                <div className="flex items-center justify-between">
                                    <span className="font-bold flex items-center gap-1.5"><Zap className="w-3 h-3 text-amber-400"/> Trigger</span>
                                    <span className="font-mono text-foreground text-right">{wf.trigger}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="font-bold flex items-center gap-1.5"><Repeat className="w-3 h-3 text-blue-400"/> Runs</span>
                                    <span className="font-mono text-foreground">{wf.runs.toLocaleString('de-DE')}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="font-bold flex items-center gap-1.5"><Timer className="w-3 h-3 text-purple-400"/> Last Run</span>
                                    <span className="font-mono text-foreground">{wf.lastRun}</span>
                                </div>
                            </div>
                        </Card>
                    );
